...
def fuck():
    ...
    attention_layer = attention(30).train()
    #total_motif = dict()
    ...
    for node in range(0, data.x.shape[0], 20):
        ...
        for i in range(length):
            ...
            if motif_embedding_feature[node, :].detach().tolist() not in motif_embedding:
                ...
                idx = list(node_motifs.keys())[
                    motif_embedding.index(motif_embedding_feature[node, :].detach().tolist())]
                node_motifs[idx].append(i)
        motif_embedding = torch.Tensor(motif_embedding).to(device)
        if len(list(node_motifs.keys())) == 0:
            continue
    ...
    return total_recall / total_anser_node, total_fid / total_anser_node, total_sparse / total_anser_node, total_top1 / total_anser_node
...
for i in range(3):
    r, f, s, t = fuck()
    fid[f'fidelity_{i}'] = f.item()
    recall[f'recall_{i}'] = r
    spar[f'spr_{i}'] = s
    top1[f'top1_{i}'] = t
